...
const App = () => {
  ...
  const changeUsernameFunc = (event) => {
    changeUsername(event.target.value);
  };

  const changePasswordFunc = (event) => {
  ...
  const authenticate = async (event) => {
    ...
    try {
      ...
      // Optionally, store JWT token or handle response
      // localStorage.setItem("token", response.data.jwtToken);
    } catch (error) {
      console.error("Error in Logging in:", error);
      alert(`Failed to Login. Please try again. Error: ${error.message}`);
    }
  };
  ...
  return (
    <div className="app-container">
      {/* Login */}
      <form className="login-container" onSubmit={authenticate}>
        <input
          type="text"
          name="username"
          placeholder="Username"
          value={username} // Bind the value to state
          onChange={changeUsernameFunc}
        />
        <input
          type="password"
          name="password"
